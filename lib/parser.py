    ...
    for line in texto.split('\n'):
        ...
        for sent in line.split('. '):
         ...
         # print t,t_clean,start,first
                ...
                elif not t_clean.isdigit() and t_clean.upper() \
                    == t_clean and 0 < len(t_clean) < 4 and t_clean \
                    not in ['I', 'X', 'Y', 'CGH']:
           ...
           # print start,texto[start:pos+20]
                    ...
                    print token, t_clean, '->', pos, texto[pos:pos
                        + len(t_clean)]
                    print pos - offset
                elif not first and not t_clean.isdigit() \
                    and len(t_clean) == 2 and t_clean[0].capitalize() \
                    == t_clean[0]:
...
def split_per_name(total, texto):  # NOW not used -> find_associations!
    ...
    while i < len(total):
        ...
        elif code_ant == 1 and code_actual != 1:

                                             # empieza info de una persona
...
def find_associations(total, texto):
  ...
  # pivot elements (mainly observations and hpo annotations)
    ...
    obsv = [0] + [total.index(ann) for ann in total if ann[0] in [0,
                  2]] + [len(total)]
    patient_ant = ''
    for i in range(1, len(obsv) - 1):
      ...
      # look for associations in the nearest entities within the boundaries
        ...
        poscs = find_partners(total, obsv[i], start=obsv[i - 1],
                              end=obsv[i + 1])
      ...
      # determine the patient and group associations around it
        ...
        if True:
            ...
            if association[0] != '':
                ...
                patient = patient_ant
            if len(patient) > 0:
                if output.has_key(patient):
                    output[patient].append(association)
                else:
                    output[patient] = [association]
...
def find_partners(
    ...
    for pos in range(start + 1, end):
        ...
        if code != 2 and code > 0 and (uid == uid_ref or uid == ''):
            if candidates.has_key(code):
                candidates[code].append(pos)
            else:
                candidates[code] = [pos]
    for k in candidates:
...
def choose_one(lann):
    ...
    if len(out) == 0:
        if len(set(['ANAT', 'CELL', 'GENE']) & sgrs):
            return [ann for ann in lann if ann['grp'].split('.')[0]
                    in ['ANAT', 'CELL', 'GENE']]
        else:
            return [min(lann)]
    else:
...
def choose_color_code(src, grp):
    ...
    if src == u'shifters':
        ...
        color = '#BD7AFF'
        code = 2
    elif src in [u'umls2012ab_gene_disorders', u'umls2012ab_omim',
                 u'umls2012ab_hpo', u'umls2012ab_congenital'] and grp \
        in ['DISO', 'OBSV', 'ACTI', 'CONC']:
        color = '#FA5858'
        code = 2
    ...
    # print a['match'],a['src'],a['grp'],a['offset']
        ...
        if offset == offset_ant:
            if pos != -1:
                ...
                print 'Warning: annotation not found in text ->', \
                    a['match']
...
def extract_all(texto):
 ...
 # group by sentence
    ...
    for (k, grupo1) in iter.groupby(Annotations, key=lambda x: x['id'
                                    ].split('.')[:-1]):
     ...
     # group annotations with same offset and length
        ...
        for (k2, grupo2) in iter.groupby(sorted(grupo1, key=lambda x: \
                (x['offset'], -x['len'])), key=lambda x: (x['offset'],
                x['len'])):
        ...
        # print k2, anns
            ...
            for ann in anns:
             ...
             # merge ann_ant
                    ...
                    ann_ant = procAnn[-1]
                    (new_color, new_code) = choose_color_code(ann['src'
                            ], ann['grp'])
                    if new_color != '':
                        match = (ann_ant['match'])[:offset
                            - offset_ant] + ann['match']

                # start=max(offset_ant-3,0)
                # pos=texto.find(match,start)

                        procAnn[-1]['match'] = match
                        procAnn[-1]['len'] = len_ant - (offset_ant
                                - offset)
             ...
             # u'match': u'palmar creases'}
                    ...
                    if texto[ann['offset']:ann['offset'] + ann['len']] \
                        != ann['match']:
                        print '\tWarning: some offset misalignment ?', \
                            ann['match'],
                        print ann['offset'], texto[ann['offset'
                                ]:ann['offset'] + ann['len']]
                    (color, code) = choose_color_code(ann['src'],
                            ann['grp'])
                    if code != -1:
                        ann['color'] = color
                        ann['code'] = code
    ...
    total.extend(map(lambda x: [x[k] for k in [
            'code',
            'offset',
            'match',
            'color',
    ...
    for t in total:  # preserve the order of insertion in total
        if t[0] == 0:  # #fake annotation for association finding
            continue
        pos = t[1]
   ...
   # print "::",pos,texto[pos:pos+t[-1]]
        ...
        if dicc_res.has_key(pos):
            if len(dicc_res[pos]) == 1 and dicc_res[pos][0][0] == 1:  # si hay una etiqueta de persona, descartamos las demas
                pass
            elif t not in dicc_res[pos]:

                                  # no repetir anotaciones
